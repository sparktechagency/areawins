...
export default function SportsBettingInterface({
  ...
  const activeConfig =
    config[sportKey as keyof typeof config] || config.football;
  ...
  const matches: Match[] = [
    ...
    },
    {
      ...
      odds: { home: 5.8, draw: 0, away: 1.12 },
      markets: 8,
    },
    // VOLLEYBALL
    {
    ...
    <div className="flex flex-col xl:flex-row gap-6 h-screen bg-background text-white p-4 overflow-hidden pt-20">
      ...
      <div className="w-64 hidden 2xl:block shrink-0 space-y-6 pr-4 border-r border-white/10 overflow-y-auto no-scrollbar">
        ...
        <div className="space-y-1 pt-4 border-t border-white/10">
          <h3 className="text-xs font-bold text-white/50 uppercase tracking-wider mb-2 px-3">
            All Countries
          </h3>
      ...
      <div className="flex-1 overflow-y-auto pr-2 pt-5 no-scrollbar">
        ...
        <div className="relative rounded-2xl overflow-hidden mb-8 min-h-60 flex flex-col justify-end">
          ...
          <div className="relative z-10 p-8 max-w-2xl">
            <div className="flex items-center gap-3 mb-2">
              <Badge className="bg-primary text-black hover:bg-primary font-bold border-none">
                {activeConfig.banner.promo?.includes("Bonus")
                  ? "PROMOTION"
                  : "SUPER BOOST"}
              </Badge>
            ...
            <Button
              size="lg"
              className="bg-white text-black hover:bg-gray-100 font-bold border-none px-8"
            >
            ...
            {sportKey === "football" && (
              <div className="bg-card p-3 rounded-lg border border-border relative group">
                ...
                <div className="text-xs text-primary mb-1 flex items-center gap-2">
                  <span className="bg-red-500/20 text-red-500 px-1 rounded text-[10px]">
                    LIVE
                  </span>
                  Match Result
                </div>
      ...
      >
          ...
          {match.isLive && match.score && (
            <>
              <span className="text-primary">
                {typeof match.score.time === "string" ? match.score.time : ""}
              </span>
            </>
          )}
        </div>
        ...
        <div className="grid grid-cols-[120px_1fr] md:grid-cols-[160px_1fr] gap-4 mb-2">
          ...
          <div className="flex items-center">
            <span className="font-bold text-primary text-lg ml-auto md:ml-0 md:pl-8">
              {typeof match.score?.home === "string"
                ? match.score.home
                : match.score?.home}
            </span>
          </div>
          ...
          <div className="flex items-center">
            <span className="font-bold text-primary text-lg ml-auto md:ml-0 md:pl-8">
              {typeof match.score?.away === "string"
                ? match.score.away
                : match.score?.away}
            </span>
          </div>
        ...
        {sport === "cricket" && match.score && (
          <div className="text-xs text-gray-400 mt-2 ml-10">
            {match.homeTeam}: {match.score.home}, {match.awayTeam}:{" "}
            {match.score.away}
          </div>
        )}
      </Link>
