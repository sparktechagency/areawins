          ...
          <div className="flex-1">
             ...
             <div className="relative rounded-xl overflow-hidden bg-card border border-border mb-6">
                 ...
                 <div className="relative z-10 p-6 md:p-8">
                     ...
                     <div className="flex flex-col md:flex-row items-center justify-between gap-8">
                         ...
                         <div className="text-center flex-1">
                            <div className="w-20 h-20 mx-auto bg-muted rounded-full flex items-center justify-center mb-3 shadow-lg border-4 border-background">
                               <span className="text-2xl font-bold">M</span>
                            </div>
                            <h2 className="text-2xl font-bold text-foreground">{match.homeTeam}</h2>
                         </div>
                         ...
                         <div className="flex flex-col items-center">
                             {match.isLive ? (
                                <>
                                   ...
                                   <Badge variant="destructive" className="animate-pulse px-3 py-1">LIVE • {match.score.time}</Badge>
                                </>
                             ) : (
                                <>
                         ...
                         <div className="text-center flex-1">
                            ...
                            <h2 className="text-2xl font-bold text-foreground">{match.awayTeam}</h2>
                         </div>
                     </div>
                 </div>
                 ...
                 {match.isLive && (
                    <div className="border-t border-border bg-muted/30 p-4 grid grid-cols-3 gap-4 text-center text-xs text-muted-foreground">
                        <div>
                           <div className="font-bold text-foreground mb-1">Possession</div>
                           <div className="flex items-center gap-2 justify-center">
                              <span>{match.stats.possession.home}%</span>
                              <div className="w-20 h-1.5 bg-muted rounded-full overflow-hidden">
                                 <div className="h-full bg-primary" style={{ width: `${match.stats.possession.home}%` }} />
                              </div>
                              <span>{match.stats.possession.away}%</span>
                           </div>
                        </div>
                        <div>
                           <div className="font-bold text-foreground mb-1">Shots on Target</div>
                           <div className="flex items-center gap-2 justify-center">
                              <span>{match.stats.shots.home}</span>
                              <div className="w-20 h-1.5 bg-muted rounded-full overflow-hidden">
                                  <div className="h-full bg-blue-500" style={{ width: `${(match.stats.shots.home / (match.stats.shots.home + match.stats.shots.away)) * 100}%` }} />
                              </div>
                        ...
                        <div>
                           ...
                           <div className="flex items-center gap-2 justify-center">
                              ...
                              <div className="w-20 h-1.5 bg-muted rounded-full overflow-hidden">
                                  <div className="h-full bg-orange-500" style={{ width: `${(match.stats.corners.home / (match.stats.corners.home + match.stats.corners.away)) * 100}%` }} />
                              </div>
             ...
             <Tabs defaultValue="all" className="space-y-6">
                <TabsList className="w-full justify-start bg-transparent border-b border-border rounded-none h-auto p-0 gap-6">
                   <TabsTrigger value="all" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent px-0 pb-2">All Markets</TabsTrigger>
                   <TabsTrigger value="main" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent px-0 pb-2">Main</TabsTrigger>
                   <TabsTrigger value="goals" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent px-0 pb-2">Goals</TabsTrigger>
                   <TabsTrigger value="half" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent px-0 pb-2">Half Time</TabsTrigger>
                </TabsList>
         ...
         <CardContent className="p-4">
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
               {market.options.map((option: any, idx: number) => (
                  <button key={idx} className="flex justify-between items-center p-3 rounded-lg border border-border hover:border-primary hover:bg-primary/5 transition-all group">
                     <span className="text-sm text-foreground">{option.label}</span>
                     <span className="font-bold text-primary">{option.odds.toFixed(2)}</span>
                  </button>
               ))}
            </div>
         </CardContent>
