...
export const matchApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    // Get live matches
    getLiveMatches: builder.query<MatchesResponse, MatchFilter | void>({
      query: (filter) => ({
        url: "/matches/live",
        params: filter,
      }),
    ...
    getUpcomingMatches: builder.query<MatchesResponse, MatchFilter | void>({
      query: (filter) => ({
    ...
    getMatchesBySport: builder.query<MatchesResponse, { sport: SportType; filter?: MatchFilter }>({
      query: ({ sport, filter }) => ({
        url: `/matches/sport/${sport}`,
        params: filter,
      }),
      providesTags: (result, error, { sport }) => [{ type: "Matches", id: sport }],
    }),

    // Get tournaments
    getTournaments: builder.query<{ tournaments: Tournament[] }, { sport?: SportType }>({
      query: (params) => ({
        url: "/tournaments",
        params,
      }),
    ...
    getTournamentById: builder.query<{ tournament: Tournament }, string>({
      ...
      providesTags: (result, error, tournamentId) => [{ type: "Tournaments", id: tournamentId }],
    }),

    // Get tournament standings
    getTournamentStandings: builder.query<TournamentStandings, string>({
      query: (tournamentId) => `/tournaments/${tournamentId}/standings`,
      providesTags: (result, error, tournamentId) => [{ type: "Tournaments", id: `${tournamentId}-standings` }],
    }),

    // Get teams
    getTeams: builder.query<{ teams: Team[] }, { sport?: SportType; search?: string }>({
      query: (params) => ({
        url: "/teams",
        params,
      }),
    ...
    getMatchStatistics: builder.query<MatchStatistics, string>({
      ...
      providesTags: (result, error, matchId) => [{ type: "Matches", id: `${matchId}-stats` }],
    }),
    ...
    getMatchesByTournament: builder.query<MatchesResponse, { tournamentId: string; filter?: MatchFilter }>({
      query: ({ tournamentId, filter }) => ({
        url: `/matches/tournament/${tournamentId}`,
        params: filter,
      }),
      providesTags: (result, error, { tournamentId }) => [{ type: "Matches", id: `tournament-${tournamentId}` }],
    }),

    // Get team matches
    getTeamMatches: builder.query<MatchesResponse, { teamId: string; filter?: MatchFilter }>({
      query: ({ teamId, filter }) => ({
        url: `/matches/team/${teamId}`,
        params: filter,
      }),
      providesTags: (result, error, { teamId }) => [{ type: "Matches", id: `team-${teamId}` }],
    }),

    // Search matches
    searchMatches: builder.query<MatchesResponse, { query: string; filter?: MatchFilter }>({
      query: ({ query, filter }) => ({
        url: "/matches/search",
